    ...
    if (!course) {
        return (
            <div className="text-center py-24">
                ...
                <Link to="/courses" className="text-fuchsia-400 font-semibold">← Back to Courses</Link>
            </div>
        )
    }
        ...
        <div className="bg-slate-950">
            <div className="relative h-[40vh]">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white p-4">
                    <span className="bg-gradient-to-r from-violet-600 to-fuchsia-500 text-white text-sm font-semibold px-4 py-1.5 rounded-full mb-4">Course</span>
                    ...
                    <div className="space-y-8">
                        <div className="bg-slate-900 p-6 rounded-xl border border-white/10">
                            ...
                            <div className="space-y-3 text-slate-300">
                                {details.duration && <div className="flex justify-between"><span>Duration</span><span className="font-semibold">{details.duration}</span></div>}
                                {details.level && <div className="flex justify-between"><span>Level</span><span className="font-semibold">{details.level}</span></div>}
                                {details.tools?.length > 0 && (
                                    <div>
                                        <span className="block mb-2">Tools</span>
                                        <div className="flex flex-wrap gap-2">
                                            {details.tools.map((t, i) => (
                                                <span key={i} className="px-3 py-1 rounded-full text-xs bg-slate-800 text-slate-200 border border-white/10">{t}</span>
                                            ))}
                                        </div>
                                    </div>
                                )}
                            ...
                            <button onClick={() => window.location.assign("/consultation")} className="mt-6 w-full bg-gradient-to-r from-violet-600 to-fuchsia-500 text-white py-3 rounded-lg font-semibold text-lg hover:-translate-y-0.5 transition-transform duration-300">
                                Apply Now
                            </button>
                        </div>
                    </div>
